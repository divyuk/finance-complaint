...
class DataIngestion:
    ...
    def get_required_interval(self):
        """
        Based on from date and to date we are going to get the data.
        Returns:
            intervals: list
        """
    ...
    def download_files(self, n_day_interval_url: int = None):
        ...
        """
        try:
            required_interval = self.get_required_interval() # we have the interval
            logger.info("Started downloading files")
            # we Will loop through the date range and will get the data
    ...
    def write_metadata(self, file_path: str) -> None:
        ...
        that is next time "to" date will become from date and
        to date will become the running date.
        """
        try:
    ...
    def initiate_data_ingestion(self) -> DataIngestionArtifact:
        try:
            ...
            # from date and to date needs to be different for data to download.
